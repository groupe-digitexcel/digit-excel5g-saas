  ...
  return (
    <div className="space-y-8 animate-fade-up">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Utilisateurs', value: usersCount ?? 0, icon: '👤', color: '#0F4CFF' },
          { label: 'Paiements', value: paymentsCount ?? 0, icon: '💳', color: '#F5B301' },
          { label: 'Travaux IA', value: jobsCount ?? 0, icon: '🤖', color: '#00D4FF' },
          { label: 'En attente', value: pendingCount ?? 0, icon: '⏳', color: '#FF4D4F' },
        ].map(({ label, value, icon, color }) => (
          <div key={label} className="card p-5 text-center">
            <div className="text-3xl mb-2">{icon}</div>
            <div className="section-title text-3xl mb-0.5" style={{ color }}>
              {value}
            </div>
              ...
              <thead>
                <tr className="border-b border-surface">
                  {[
                    ...
                    <th
                      key={h}
                      className="px-5 py-3 text-left text-xs font-semibold text-white/35 uppercase tracking-wider whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
