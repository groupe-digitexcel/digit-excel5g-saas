...
export async function POST(req: Request) {
  ...
  if (!brand || !offer) {
    return NextResponse.json(
      { error: 'Marque et offre requises' },
      { status: 400 }
    )
  }
...
JSON: {"headline":"titre court accrocheur","subtitle":"sous-titre 15 mots max","body":"texte principal 2-3 phrases","cta":"appel action 6 mots","imagePrompt":"description image IA en anglais style africain professionnel"}`
  ...
  try {
    ...
    result = {
      headline: `${brand} — ${offer}`,
      subtitle: 'La meilleure offre de la saison',
      body:
        'Profitez de cette offre exceptionnelle. Qualité garantie, service professionnel. Ne manquez pas cette opportunité!',
      cta: 'Contactez-nous maintenant!',
      imagePrompt: `Professional ${type} advertising for ${brand} in Cameroon, African style`,
    }
  }
